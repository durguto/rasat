...
Use GitHub's private vulnerability reporting on this repository: **Security → Report a vulnerability**. If that button is missing, contact the maintainer via [github.com/durguto](https://github.com/durguto).
...
Only the latest tagged release on Docker Hub (`durguto/rasat`) is patched. Older tags are not backported; the fix lands on `main` and ships in the next patch. See [CONTRIBUTING.md](CONTRIBUTING.md).
