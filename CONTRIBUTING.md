...
| Bump | Use for |
|---|---|
| **PATCH** (`v0.1.3`) | Fixes, docs, internals. No intended break to env vars, HTTP query API, OTLP paths, or the image contract. |
...
That tag builds `durguto/rasat:0.1.3`, `:0.1`, and `:latest` (`linux/amd64`, `linux/arm64`) and opens the GitHub Release. The same tag dispatches [durguto/rasat-demo](https://github.com/durguto/rasat-demo) so the hosted cassette matches the image. A failed Worker must not fail Hub.

In the same PR that you are about to tag (or immediately after):

1. Move **Unreleased** items under `## 0.1.3 — YYYY-MM-DD` (tag without the `v`, date of the tag in UTC). Group as **Added**, **Changed**, **Fixed**, or **Removed**. Omit empty groups. Write for operators: what they see or must do, not internals.
